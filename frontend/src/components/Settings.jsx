        ...
        <section className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-squircle-lg p-6 md:p-8 shadow-xl">
          ...
          <div className="flex flex-wrap items-end gap-6">
            <div className="flex flex-col gap-2 w-full sm:w-[200px]">
              <label className="text-sm font-light text-dim/80 ml-1 uppercase tracking-wider">Seconds per phase</label>
              <input 
                type="number" 
                value={boxPattern[0]} 
                onChange={(e) => handleChange(e.target.value)}
                min="0"
                className="bg-white/5 border border-white/10 rounded-squircle-sm p-4 text-text text-center text-xl font-light focus:outline-none focus:border-accent transition-all"
              />
            </div>
            <button 
              onClick={handleSave} 
              className="w-full sm:w-auto btn-primary tracking-wide h-[62px] flex items-center justify-center gap-2"
            >
              <Save size={18} />
              Save Pattern
            </button>
